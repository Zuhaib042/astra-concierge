...
- UX polish complete: added an animated three-dot typing indicator while Astra is preparing or streaming a response before visible assistant text appears.
- Admin Dashboard v1 complete: added a dynamic `/admin` route with a read-only operations dashboard. It summarizes conversations, leads, messages, knowledge-base embedding coverage, source document categories, top lead intents, recent conversations, and recent saved messages from Neon. This route must be protected with authentication or Vercel project protection before sharing a production deployment with real/private data.
- Public demo polish complete: replaced developer-facing homepage/chat labels with client-facing copy, added visible admin dashboard links, added a detailed About section for potential clients, hid public source/lead technical badges, and updated Astra's prompt so it does not expose internal source IDs such as `[S1]` in visitor responses. Future public UI work should continue to explain business outcomes without showing provider names, route names, embeddings, chunk labels, or other internal implementation details.
- Timezone formatting cleanup complete: added `date-fns` and `date-fns-tz`, centralized dashboard timestamp formatting in `src/lib/time/format.ts`, added curated timezone options in `src/lib/time/options.ts`, and added a styled dashboard timezone select that stores the chosen display timezone in the `tz` URL query parameter. `APP_TIME_ZONE` remains the default fallback for deployments.
